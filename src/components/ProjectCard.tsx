...
const ProjectCard = ({ title, technologies, description, codeLink, liveLink }: ProjectCardProps) => {
	...
	return (
		<div
			className=' duration-500 hover:translate-x-2 grid grid-cols-2  rounded-2xl  overflow-hidden bg-black border hover:border-sky-600/50 border-white/[0.2] group-hover:border-slate-700
         my-5'>
			<Image src={'/111.png'} width={500} height={120} alt='project photo' className='p-2 rounded-3xl' />
			<div className='flex flex-col items-start p-4 justify-evenly'>
				<h3 className='text-xl font-bold text-white'>{title}</h3>
				<p className='text-sm text-sky-600'>{newTechnologies}</p>
				<p className=' text-sm max-w-[450px]'>{description}</p>
